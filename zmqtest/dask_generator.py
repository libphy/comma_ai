  ...
  #for cword, tword in zip(camera_names, logs_names):
    try:
     # with h5py.File(tword, "r") as t5:
      ...
      import traceback
      traceback.print_exc()
      print("failed to open", cword) #tword

  #angle = np.concatenate(angle, axis=0)
  #speed = np.concatenate(speed, axis=0)
  #filters = np.concatenate(filters, axis=0).ravel()
  print("open train data")
  #print("training on %d/%d examples" % (filters.shape[0], angle.shape[0]))
  return c5x, y #c5x, angle, speed, filters, hdf5_camera


first = True


def datagen(filter_files, time_len=1, batch_size=10, ignore_goods=False):
  """
  Parameters:
  -----------
  ...
  c5x, y = concatenate(filter_names, time_len=time_len)
  ...
  y_batch = np.zeros((batch_size, time_len, 1), dtype='uint8')
  ...
  try:
    ...
    #     speed_batch[count] = np.copy(speed[i-time_len+1:i+1])[:, None]
      #
    #     count += 1
      #print(type(c5x), len(c5x), c5x[0].shape)
      for offset in range(0, c5x[0].shape[0], batch_size):
          X_batch = c5x[0][offset:offset+batch_size]
    ...
    #   print("%5.2f ms" % ((time.time()-start)*1000.0))
        ...
        # print("speed", speed_batch.shape)
          ...
          yield (X_batch, y_batch) #(X_batch, angle_batch, speed_batch)
      i+=1
